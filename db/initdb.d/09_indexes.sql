-- Extensions first (safe to re-run)
...
-- --------------------- inv_jsonb (payload) ---------------------
-- GIN containment
CREATE INDEX IF NOT EXISTS inv_jsonb_payload_gin_path
  ON inv_jsonb USING GIN (payload jsonb_path_ops);
...
-- Timestamps as ISO strings (lexicographic == chronological)
...
-- Composite (optional)
CREATE INDEX IF NOT EXISTS inv_jsonb_idx_text1_ts1_num1_str
  ON inv_jsonb ((payload->>'indexed_text_1'),
                (payload->>'indexed_timestamp_1'),
                ((payload->>'indexed_number_1')::numeric));

CREATE INDEX IF NOT EXISTS inv_jsonb_idx_text1_ts1desc_str
  ON inv_jsonb ((payload->>'indexed_text_1'),
                (payload->>'indexed_timestamp_1') DESC);
